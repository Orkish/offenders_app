...
area_seed.each do |key,value|
	...
	value.each do |v|
		...
		puts "now in #{v}"
		person_name = []
		puts "person is reinitialized"
		page = Nokogiri::HTML(open("http://www.criminaljustice.ny.gov/SomsSUBDirectory/search_index.jsp?offenderSubmit=true&LastName=&County=&Zip=#{v}&Submit=Search"))
